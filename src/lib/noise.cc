...
double noise::simplex::generate(double x, double y) const {
	...
	if constexpr (CLAMP_OUTPUTS) b::clamp(v, -1.0, 1.0);
	return v;
}
...
double noise::simplex::generate(double x, double y, double z) const {
...
double noise::simplex::generate(double x, double y, double z, double w) const {
	...
	t = 0.6 - b::vec4d::dot(c0, c0);
	nc[0] = t < 0 ? 0 : std::pow(t, 4) * b::vec4d::dot(GRAD4[gi[0]], c0);
	t = 0.6 - b::vec4d::dot(c1, c1);
	nc[1] = t < 0 ? 0 : std::pow(t, 4) * b::vec4d::dot(GRAD4[gi[1]], c1);
	t = 0.6 - b::vec4d::dot(c2, c2);
	nc[2] = t < 0 ? 0 : std::pow(t, 4) * b::vec4d::dot(GRAD4[gi[2]], c2);
	t = 0.6 - b::vec4d::dot(c3, c3);
	nc[3] = t < 0 ? 0 : std::pow(t, 4) * b::vec4d::dot(GRAD4[gi[3]], c3);
	t = 0.6 - b::vec4d::dot(c4, c4);
	nc[4] = t < 0 ? 0 : std::pow(t, 4) * b::vec4d::dot(GRAD4[gi[4]], c4);
